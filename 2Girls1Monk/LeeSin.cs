...
namespace two_girls_one_monk
{
    class LeeSin
    {
        ...
        {
            var minion = ObjectManager.Get<Obj_AI_Minion>().Where(minions => minions.Distance(Player) < 1000);
        ...
        public static void insec1()
        {
            ...
            try
            {
                ...
                if (Player.Distance(getward2(LockedTarget)) > 600 && Player.Distance(getward2(LockedTarget)) <= Q.Range && W.IsReady())
                {
                    castQFirstSmart();
                    castQSecondSmart();
                }
        ...
        {
            foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsValidTarget(R.Range) && Damage.GetSpellDamage(Player, hero, SpellSlot.R) >= hero.Health))
            {
        ...
        public static void castRKill()
        {
            ...
            if (!(LockedTarget != null))
                return;
            if (Damage.GetSpellDamage(Player, LockedTarget, SpellSlot.R) <= LockedTarget.Health)
                return;
            if (!R.IsReady())
                return;
            R.Cast(LockedTarget);
        ...
        public static bool putWard(Vector2 pos)
        {
            ...
            if ((wardItem = getJumpWardId()) != -1)
            {
                ...
                {
                    Player.Spellbook.CastSpell(Items.GetWardSlot().SpellSlot ,pos.To3D());
                    return true;
                }
            }
            return false;
