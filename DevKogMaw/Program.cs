...
namespace DevKogMaw
{
    class Program
    {
        ...
        {
            if (mustDebug)
                Game.PrintChat(string.Format("OnPosibleToInterrupt -> {0} cast {1}", sender.SkinName));


            var EInterrupt = Config.Item("EInterrupt").GetValue<bool>();
            var packetCast = Config.Item("PacketCast").GetValue<bool>();

            if (EInterrupt && E.IsReady() && sender.IsValidTarget(E.Range))
            {
                E.CastIfHitchanceEquals(sender, sender.IsMoving ? HitChance.High : HitChance.Medium, packetCast);
            }
        }
        ...
        private static void OnDraw(EventArgs args)
        {
            foreach (var spell in SpellList)
            {
                ...
                if (menuItem.Active && spell.IsReady())
                {
                    ...
                    {
                        if (spell.IsReady())
                            Render.Circle.DrawCircle(ObjectManager.Player.Position, Player.AttackRange + spell.Range, System.Drawing.Color.Green);
                        else if (HasPassiveBuff())
                            Render.Circle.DrawCircle(ObjectManager.Player.Position, Player.AttackRange, System.Drawing.Color.Orange);
                        else
                            Render.Circle.DrawCircle(ObjectManager.Player.Position, Player.AttackRange + spell.Range, System.Drawing.Color.Red);
                    }
                    else
                    {
                        if (spell.IsReady())
